...
class Controller:
    ...
    ##fcu_horizon is frame wrt to drone
...
##local_origin is frame wrt to world origin


def move_to_destination(x,y,z,yaw,frame_id='local_origin'):
    rospy.loginfo('moving to destination...')
    rospy.wait_for_service('set_position')
    try:
        set_position = rospy.ServiceProxy('set_position', SetPosition)
...
def main():
    ...
    while not rospy.is_shutdown():
        print(cnt.local_pos)
        move_to_destination(0,0,2,0)
        rospy.sleep(3)
        move_to_destination(2,0,2,0)
        rospy.sleep(3)
        move_to_destination(2,2,2,0)
        rospy.sleep(3)
    	rate.sleep()
